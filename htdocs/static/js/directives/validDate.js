...
angular.module('app').directive('validDate',
function ($filter, $window, $parse, $timeout) {
        ...
        return function (scope, element, attrs, ngModel) {
            ...
            if (ngModel) {
                element.on('blur', function (event) {
                    var date = moment($filter('date')(element.val(), "dd/MM/yyyy"),'DD/MM/YYYY');
                    // if the date entered is a valid date then save the value
                    if (date && moment(element.val(), "DD/MM/YYYY").isValid() && date >= moment("01/01/1900")) {
