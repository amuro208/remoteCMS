      ...
      if (strlen($backup)) {
        if (!write_file($file, $backup)) {
          ...
          if ($saved_data !== NULL) {
            ...
            $this->handle_success('You can download db backup here ' . anchor($this->db_download_url . $saved_data, 'download', array(
              'class' => 'download'
            )));
            $this->handle_success('You can delete db backup here ' . anchor($this->db_delete_url . $saved_data, 'delete', array(
              'class' => 'delete',
    ...
    if ($file_format == 1) {
      ...
      if ($zipped == 1) {
        ...
        if ($saved_data !== NULL) {
          ...
          $this->handle_success('You can get site backup here ' . anchor($this->site_download_url . $saved_data, 'download', array(
            'class' => 'download'
          )));
          $this->handle_success('You can delete site backup here ' . anchor($this->site_delete_url . $saved_data, 'delete', array(
            'class' => 'delete',
            'onclick' => "return confirm('Are you sure want to delete this file ?')"
          )));
        } else {
