...
class ImageManipulator
{
     ...
     * Merge image
     *
     * @param string $file the path of frame file.
     * @param object $dm dimension object.
     *               x: x position in frame
     *               y: y position in frame
     *               cx :
     *               cy :
     *               cw : contents width in frame
     *               ch : contents heigth in frame
     *               x1 : crop x1
     *               x1 : crop y1
     *               x2 : crop x2
     *               y2 : crop y2
     ...
     * @throws RuntimeException
     */
    public function merge($file,$dm){
        if (!is_resource($this->image)) {
            throw new RuntimeException('No image set');
        }

        $frame = new ImageManipulator($file);
        $this->crop($dm->x1,$dm->y1,$dm->x2,$dm->y2)
             ->resample($dm->cw,$dm->ch,true);

        $frame = $frame->getResource();
        imagecopyresampled(
          $frame,
          $this->image,
          $dm->x, $dm->y,
